...
trait Helpers { self: D =>
  ...
  extension[T] (x: T)
    ...
    def part: Unit =
      ...
      answer(level, x.toString).thenDo(_.pr("response = "))
    def pr(pfx: String = ""): Unit = if debug_print then println(pfx + " " + x)
  def pLines(ls: Any*): Unit = if debug_print then ls foreach println
...
trait D extends Helpers:
  ...
  def answer(level: Int, answer: String) = 
    val lookFor = "<p>([^<]+)".r
    ...
    if txt.contains(answer) then None else
...
object Answer {
  ...
  trait SimpleAnswer(val mach: String) extends Answer
  ...
  object SimpleAnswer:
    val all = List[SimpleAnswer](NotRight, Correct)

  def from(s: String): Answer = SimpleAnswer.all.find(_.mach == s).getOrElse(s match
